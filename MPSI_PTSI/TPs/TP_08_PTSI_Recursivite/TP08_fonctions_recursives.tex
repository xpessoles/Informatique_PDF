...
\begin{equation}
\pi= 2  \prod_{i=1}^n \frac{4i^2}{4i^2-1}= 2 \times \frac{4}{3} \times \frac{16}{15}\times  \frac{36}{35}\times ...
\label{"Wallis"}
\end{equation}

\item la série de Gregory :

\begin{equation}
\pi= 4  \sum_{i=0}^n \frac{(-1)^i}{2i+1}=4 ( 1 - \frac{1}{3} + \frac{1}{5} -  \frac{1}{7} + ... )
\label{GREG"}
\end{equation}
...
\question{Implémenter la fonction \texttt{Wallis\_it(n:int) -> float} qui permet de calculer $\pi$ par une méthode \textbf{itérative}. Tester pour \textbf{n=10} (on trouve environ 3.0677).}

\question{Reprendre la question précédente avec une méthode \textbf{récursive}. Tester pour \textbf{n=10}.}

\question{Implémenter la fonction \texttt{Gregory\_it(n:int) -> float} qui permet de calculer $\pi$ par une méthode \textbf{itérative}. Tester pour \textbf{n=10} (on trouve environ 3.232).}
...
On souhaite réaliser une fonction \texttt{miroir(m:str)} dont le but est de retourner le <<miroir>> d'une chaîne de caractères. 
Par exemple le résultat de \texttt{miroir("miroir")} serait \textsl{''riorim''}.

\bigskip
\question{Programmer la fonction \texttt{miroir\_rec(m:str)} permettant de répondre au problème de manière récursive.} 
...
Une expression qui se lit de manière identique dans les deux sens est appelée \textbf{"palindrome"}. C'est le cas de : "radar", "esope reste et se repose", "kayak" ...

\question{Ecrire une fonction récursive nommée \texttt{est\_palindrome\_rec(m:str)->bool} qui reçoit comme argument une chaîne de caractères contenant le mot (ou la phrase). Cette fonction doit tester si les deux caractères opposés sont identiques, puis appeler récursivement la fonction sur la partie de la chaîne qui ne contient pas ces deux caractères. La chaîne ne contiendra pas d'espaces.}
...
\question{Définir la fonction \texttt{fibonacci\_it(n:int)->int} permettant de calculer $u_n$ par une méthode itérative. }

\question{Définir la fonction \texttt{fibonacci\_rec(n:int)->int} permettant de calculer $u_n$ par une méthode récursive << 
intuitive>>. }
...
\question{\'Ecrire à la main le passage d'un point \texttt{n} au suivant \texttt{n+1} en fonction de \texttt{x} et \texttt{y}. Faire de même, dans le sens inverse avec \texttt{x} et \texttt{y} connus.}

\question{\'Ecrire une fonction récursive \texttt{point(x:int,y:int)->int} qui renvoie le numéro du point de coordonnées $(x,y)$.}

\question{\'Ecrire la fonction réciproque \texttt{coordonnees(n:int)->tuple}, là encore de façon récursive.}
\end{minipage}\hfill
